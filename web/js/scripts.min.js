...
//  });
  ...
  function enableAllTheseDays(date) {
    ...
    if(typeof(daysWeek) != "undefined" && daysWeek !== null) {
      if (daysWeek[weekDay]) {
        return [true];
      }
      else {
...
$(document).ready(function () {
  ...
  $(".more-exc").click(function(event){
    ...
    var dateExc = $("#excfilter-date").val();
    var typeExc = $("#excfilter-type").val();
    var durationExc = $("#excfilter-duration").val();
    var isActiveFilter = $("#excfilter-isactive").prop('checked');

    $.ajax({
      type: "POST",
      url: '/excursions/more_exc/',
      data: "lastingExc="+id+"&date="+dateExc+"&type="+typeExc+"&duration="+durationExc+"&isActive="+isActiveFilter,
      dataType: 'json',
  ...
  $(".reviews-page__footer-btn").click(function(event){
    var id;
    $('.reviews-page-item').each(function( index ) {
      id = $(this).attr('id').toString().slice(-1);
    });
    $.ajax({
      type: "get",
      url: '/site/get_more_reviews/?last_reviews='+id,
      dataType: 'json',
      success: function(result){
        $(".reviews-page__main").append(result['code']);
        if(!result['showMore']){
          $('.reviews-page__footer').css("display","none");
        }
      }
    });
    event.preventDefault();
  ...
  $(".callback-form__submit").click(function(event){
    $.ajax({
      method: 'POST',
      type: "POST",
      url: '/request/manager_form/',
      dataType: 'html',
      processData: false,
      data: $('#form-manager').serialize(),
      success: function(result){
        $('#request-response').remove();
        $(".callback-sec").append(result);
      }
    });
    return false;
  });
